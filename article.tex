...
Пусть $C,dim(C)=n$ – множество точек, соответствующих текущему положению автотранспортных средств(АТС), $Q,dim(Q)=m$ – множество пунктов назначения. Прибытие в пункт назначения должно быть выполнено в промежутке времени $[t_j,t_j + \Delta t_j]$. Временные затраты, которые могут быть связаны с разгрузкой товаров, равны $w_j, \forall j \in Q $. 
...
\begin{equation} \label{main_objective}
  F(X) = 
    \sum_{k \in C}[
     ...
     (\Omega_{i,j}+\varphi(t_j - t_i - \omega_i - t_{i,j})\cdot x^{(k)}_{i,j})
     ]
     \to min
\end{equation}
При ограничениях:
