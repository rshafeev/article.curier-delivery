...
\begin{figure}[h]
	\hfil\includegraphics[height=2.0in]{images/scheme.png}\hfil
	\caption
	{
...
& \sum_{i\in{V/C}\cup{k}}x_{i,s_{q}^{k}}=\sum_{i\in{V/C}\cup{k}}x_{i,r_{q}^{k}} \\
& \tilde{t}_{r}^{k}(\bar{X}^{k})-\tilde{t}_{s}^{k}(\bar{X}^{k})\geq 0,
\forall q \in Q : q=q(r,s)\\
